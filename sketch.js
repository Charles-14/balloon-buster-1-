  //To declare the variables
  var bow, arrow, scene;
  var green_balloon, red_balloon, pink_balloon, blue_balloon;
  var bowImage, arrowImage, backgroundImage;
  var green_balloonImage, red_balloonImage, pink_balloonImage ,blue_balloonImage;

  var score=0;

  //To load the images
  function preload(){
  
  backgroundImage = loadImage("background0.png");
  arrowImage = loadImage("arrow0.png");
  bowImage = loadImage("bow0.png");
  red_balloonImage = loadImage("red_balloon0.png");
  green_balloonImage = loadImage("green_balloon0.png");
  pink_balloonImage = loadImage("pink_balloon0.png");
  blue_balloonImage = loadImage("blue_balloon0.png");
}

  //To create the sprites and their functions
  function setup() {
  createCanvas(400, 400);
  
  //creating background
  scene = createSprite(0,0,400,400);
  scene.addImage(backgroundImage);
  scene.scale = 2.5;
  
  // creating bow to shoot arrow
  bow = createSprite(380,220,20,50);
  bow.addImage(bowImage); 
  bow.scale = 1;
  
  score = 0;
    
//To create the groups for balloons
redBalloonGroup = createGroup();
blueBalloonGroup = createGroup();
greenBalloonGroup = createGroup();
pinkBalloonGroup = createGroup();
arrowGroup = createGroup();
    
}


  function draw() {
  background(0);
  
  //To move the ground
  scene.velocityX = -3;

  if (scene.x < 0){
     scene.x = scene.width/2;
}
  
  //To move the bow with the mouse's X-axis
  bow.y = World.mouseY
  
  // release arrow when space key is pressed
  if (keyDown("space")) {
      createArrow();
}
   
  //creating continous balloons
  var select_balloon = Math.round(random(1,4));
  
  if (World.frameCount % 100 == 0) {
    if (select_balloon == 1) {
      redBalloon();
    } else if (select_balloon == 2) {
      greenBalloon();
    } else if (select_balloon == 3) {
      blueBalloon();
    } else {
      pinkBalloon();
    }
}  
    
   //To destroy the balloons and the arrow whenever they touch each other
   if (arrowGroup.isTouching(redBalloonGroup)){
      redBalloonGroup.destroyEach();
      arrowGroup.destroyEach();
     score = score + 1;
}
    
   if (arrowGroup.isTouching(blueBalloonGroup)){
      blueBalloonGroup.destroyEach();
      arrowGroup.destroyEach();
     score = score + 2;
}
    
   if (arrowGroup.isTouching(greenBalloonGroup)){
      greenBalloonGroup.destroyEach();
      arrowGroup.destroyEach();
     score = score + 3;
}
    
    if (arrowGroup.isTouching(pinkBalloonGroup)){
       pinkBalloonGroup.destroyEach();
       arrowGroup.destroyEach();
      score = score + 4
}
    
    
    
  drawSprites();
  textSize(17 );
  text("Score: "+ score, 300,50);
}


  // Creating  arrows for bow
  function createArrow() {
  var arrow= createSprite(100, 100, 60, 10);
  arrow.addImage(arrowImage);
  arrow.x = 360;
  arrow.y=bow.y;
  arrow.velocityX = -4;
  arrow.lifetime = 100;
  arrow.scale = 0.3;
   
  //To add the arrows in the group
  arrowGroup.add(arrow);
}

  //To create the red balloon and its functions
  function redBalloon() {
  var red = createSprite(0,Math.round(random(20, 370)), 10, 10);
  red.addImage(red_balloonImage);
  red.velocityX = 3;
  red.lifetime = 150;
  red.scale = 0.1;
    
  //To add the red balloons in the group
  redBalloonGroup.add(red);
}

  //To create the blue balloon and its functions
  function blueBalloon() {
  var blue = createSprite(0,Math.round(random(20, 370)), 10, 10);
  blue.addImage(blue_balloonImage);
  blue.velocityX = 3;
  blue.lifetime = 150;
  blue.scale = 0.1;
    
  //To add the blue balloons in the group
  blueBalloonGroup.add(blue);
}

  //To create the green balloon and its functions
  function greenBalloon() {
  var green = createSprite(0,Math.round(random(20, 370)), 10, 10);
  green.addImage(green_balloonImage);
  green.velocityX = 3;
  green.lifetime = 150;
  green.scale = 0.1;
    
  //To add the green balloons in the group
  greenBalloonGroup.add(green);
}
  
  //To create the pink balloon and its functions
  function pinkBalloon() {
  var pink = createSprite(0,Math.round(random(20, 370)), 10, 10);
  pink.addImage(pink_balloonImage);
  pink.velocityX = 3;
  pink.lifetime = 150;
  pink.scale = 1.5;
    
  //To add the pink balloons in the group
  pinkBalloonGroup.add(pink);
}
